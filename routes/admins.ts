...
router.post("/", async (req, res, next) => {
  try {
    // Verifica quantos admins já existem
    const count = await prisma.admin.count()

    // Se já existir pelo menos 1 admin, exige token
    if (count > 0) return verificaToken(req, res, next)

    // Se for o primeiro admin, segue sem token
    next()
  } catch (error) {
    res.status(500).json({ erro: "Erro ao verificar admins" })
  }
}, async (req, res) => {
